...
SELECT	*,
	CASE 
		WHEN ABS(diff_from_avg / species_avg_temp) >= 0.005 THEN 1
		ELSE 0
...
SELECT species, name,
	...
	MAX(CASE 
			WHEN is_temp_exception = 1 THEN checkup_time
			ELSE NULL
		END
	) AS latest_exception
...
-- place animals into ntiles by number of exceptions for each species; ntiles used because most closely divides species by 4
exceptions_ntile AS (
SELECT 	*,
	NTILE (4)
		OVER (PARTITION BY species 
			ORDER BY num_exceptions ASC, latest_exception DESC
		) AS ntile
FROM animal_exceptions
)
-- only look at the top ntile (top 25%)
SELECT species, name, num_exceptions, latest_exception
FROM exceptions_ntile
	WHERE ntile = 1
	ORDER BY species ASC, num_exceptions DESC, latest_exception DESC;
